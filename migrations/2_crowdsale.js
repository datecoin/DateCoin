...
const emit = async (token, emission, owner, accounts) => {
  ...
  for (const label of labels) {
    ...
    const useAddress = accounts ? accounts[labels.indexOf(label) + 1] : address;
  ...
  switch (network) {
    ...
    case 'develop':
      try {
        ...
        // Params
        const { start, end, rate } = properties.crowdsale;
        ...
        console.log(
          tools.abiEncodedParams(
            ...
            [start, end, rate, wallet, DateCoin.address, vault, preSaleVault]
          )
        );
        console.log(
          '----------------------------------------------------------------'
